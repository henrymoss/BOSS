...
#Checks for our numba implementation of the string kernel
# Fastest on a CPU
# outperformed by tf version for very large strings with GPU access
...
class Kern_Cal_Tests(unittest.TestCase):
	...
	def test_k_1(self):
		self.kern = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [1.] * 5,normalize=False,
	...
	def test_norm_diag(self):
		self.kern = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [1.] * 5,normalize=True,
									gap_decay = 2.0, match_decay = 2.0,maxlen=self.maxlen)
...
class Legacy_Tests(unittest.TestCase):
	...
	def test_compare_grads_1(self):
		self.kern_new = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [0.1, 0.2, 0.4, 0.5] ,
	...
	def test_compare_grads_2(self):
		self.kern_new = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [0.1, 0.2, 0.4, 0.5, 0.7] ,
...
class Gradient_Tests(unittest.TestCase):
	...
	def test_gradient_gap_no_norm(self):
		...
		self.kern = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [0.1,0.1],normalize=False,
									gap_decay = 0.8, match_decay = 0.8, maxlen=self.maxlen)
	...
	def test_gradient_gap_norm(self):
		...
		self.kern = StringKernel(implementation="numba", alphabet=self.alphabet,order_coefs = [0.1,0.1],normalize=True,
									gap_decay = 0.8, match_decay = 0.8, maxlen=self.maxlen)
		result = self.kern.K(self.X1, self.X2)
		true_grads = self.kern.gap_grads
