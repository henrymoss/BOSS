...
try:
	import tensorflow
	from .tf_string_kernel import TFStringKernel
except ImportError as e:
	pass  # module doesn't exist, deal with it.

from .np_string_kernel import NPStringKernel
from .numba_string_kernel import numbaStringKernel
from .GPy_string_kernel import StringKernel

class SplitStringKernel(Kern):
	"""
	Split up string and apply n string kernels with shared kernel params
	"""
	


	def __init__(self,gap_decay=1.0, match_decay=2.0, order_coefs=[1.0],
				 alphabet = [], maxlen=0, num_splits=1):

		super(SplitStringKernel, self).__init__(1,None, "splitstring")
		self._name = "splitstring"
		...
		# make new kernels for each bit
		self.kernels = []
		for i in range(0,num_splits-1):
			self.kernels.append(StringKernel(gap_decay=gap_decay,match_decay=match_decay, order_coefs=order_coefs,
				 alphabet = alphabet, maxlen=int((self.maxlen/self.num_splits))))
		# final kernel might be operating on slightly loinger string if maxlen/num_splits % !=0
		self.kernels.append(StringKernel(gap_decay=gap_decay,match_decay=match_decay, order_coefs=order_coefs,
			 alphabet = alphabet, maxlen=int((self.maxlen/self.num_splits)) + self.maxlen - self.num_splits*int((self.maxlen/self.num_splits))))
