...
class CYStringKernel(object):
    ...
    def Kdiag(self,X):   
        ...
        observed_maxlen = max([len(x[0]) for x in X])
        ...
        if self.normalize:
            ...
            X = np.array([[pad(encode_string(x[0], self.index),self.maxlen)] for x in X],dtype=np.int32)
    ...
    def K(self, X, X2=None):
        ...
        observed_maxlen = max([len(x[0]) for x in X])
        if not symmetric:
            observed_maxlen_2 = max([len(x[0]) for x in X])
        ...
        if symmetric:
            ...
            X2 = np.array([[pad(encode_string(x[0], self.index),self.maxlen)] for x in X2],dtype=np.int32)
        ...
        for i, x1 in enumerate(X):
            for j, x2 in enumerate(X2):
                ...
                if symmetric and (j < i):
                    ...
                    k_result, gap_grad, match_grad, coef_grad = _k(x1[0],x2[0],self.embs, self.maxlen, self._gap_decay, self._match_decay, self._order_coefs) 
    ...
    def _diag_calculations(self, X):
        ...
        # All set up. Proceed with kernel matrix calculations
        for i, x1 in enumerate(X):
            result = _k(x1[0],x1[0],self.embs, self.maxlen, self._gap_decay, self._match_decay, self._order_coefs)  
            k_result[i] = result[0]
...
cdef tuple _k( np.ndarray s1, np.ndarray s2, np.ndarray[np.double_t, ndim=2] embs, int maxlen, double _gap_decay, double _match_decay, list _order_coefs):
    """
    ...
    """
    cdef np.ndarray[np.double_t, ndim=1] Ki, dKi_dgap, dKi_dmatch, dk_dcoefs 
    cdef np.ndarray[np.double_t, ndim=2] S, power, tril, gaps, D, dD_dgap
    cdef np.ndarray i1, i2
    cdef np.ndarray[np.double_t, ndim=2] aux1, aux2, Kpp, daux1_dgap, daux2_dgap, daux1_dmatch, daux2_dmatch
    cdef np.ndarray[np.double_t, ndim=2] dKpp_dgap, dKpp_dmatch
    cdef np.ndarray[np.double_t, ndim=3] Kp, dKp_dgap, dKp_dmatch
    cdef int i
    cdef double match_sq, k, dk_dgap, dk_dmatch
    cdef np.ndarray[np.double_t, ndim=3] final_aux1, final_daux1_dgap, final_daux1_dmatch
    cdef np.ndarray[np.double_t, ndim=2] final_aux2, final_daux2_dgap, final_daux2_dmatch
    cdef np.ndarray[np.double_t, ndim=1] final_aux3, final_daux3_dgap, final_daux3_dmatch

    S = embs[s1].dot(embs[s2].T)

    # Make D: a upper triangular matrix over decay powers.

    power = np.ones((maxlen, maxlen))
    tril = np.zeros((maxlen, maxlen))
    i1, i2 = np.indices((maxlen, maxlen))
    for k in range(maxlen - 1):
        power[i2-k-1 == i1] = k
        tril[i2-k-1 == i1] = 1.0

    gaps = np.ones([maxlen, maxlen])*_gap_decay
    ...
    for i in range(len(_order_coefs)-1):
        aux1 = S * Kp[i]
        aux2 = aux1.dot(D[0:maxlen, 0:maxlen])
        Kpp = match_sq * aux2
        Kp[i + 1] = Kpp.T.dot(D[0:maxlen, 0:maxlen]).T

        daux1_dgap = S * dKp_dgap[i]
        daux2_dgap = daux1_dgap.dot(D[0:maxlen, 0:maxlen]) + aux1.dot(dD_dgap[0:maxlen, 0:maxlen])
        dKpp_dgap = match_sq * daux2_dgap
        dKp_dgap[i + 1] = dKpp_dgap.T.dot(D[0:maxlen, 0:maxlen]).T + Kpp.T.dot(dD_dgap[0:maxlen, 0:maxlen]).T

        daux1_dmatch = S * dKp_dmatch[i]
        daux2_dmatch = daux1_dmatch.dot(D[0:maxlen, 0:maxlen])
        dKpp_dmatch = (match_sq * daux2_dmatch) + (2 * _match_decay * aux2)
        dKp_dmatch[i + 1] = dKpp_dmatch.T.dot(D[0:maxlen, 0:maxlen]).T
