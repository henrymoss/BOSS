...
class GeneticProgrammingOptimizer(AcquisitionOptimizerBase):
	"""
	...
	def __init__(self, space: ParameterSpace, dynamic:bool = False, num_evolutions: int = 10, 
		...
		if len(space.parameters)!=1 or not isinstance(space.parameters[0],StringParameter):
			raise ValueError("Genetic programming optimizer only for spaces consisting of a single string ")
		self.space = space
		...
		self.max_length = self.space.parameters[0].max_length
		self.min_length = self.space.parameters[0].min_length
	...
	def _reproduce_then_mutate(self, parent: str,) -> str:
		...
		for i in range(0,self.space.parameters[0].length):
			# see if need to mutate child1 at poistion i
			if np.random.rand(1)[0] < self.p_mutation:
				parent[i] = np.random.choice(self.alphabet)
		return " ".join(parent)
