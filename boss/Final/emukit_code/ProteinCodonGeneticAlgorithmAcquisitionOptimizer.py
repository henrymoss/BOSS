...
# this works on the space of codons (not bases)
import logging
...
from .protein_parameter import ProteinParameter
...
class ProteinCodonGeneticProgrammingOptimizer(AcquisitionOptimizerBase):
	"""
	...
	def __init__(self, space: ParameterSpace, dynamic:bool = False, num_evolutions: int = 10, 
		...
		if len(space.parameters)!=1 or not isinstance(space.parameters[0],ProteinParameter):
		...
		self.possible_codon_swaps = self.space.parameters[0].possible_codon_swaps
	...
	def _optimize(self, acquisition: Acquisition , context_manager: ContextManager) -> Tuple[np.ndarray, np.ndarray]:
		...
		population = self.space.parameters[0].sample_uniform_codon(self.population_size)
	...
	def _evolve(self, population: np.ndarray, fitness: np.ndarray) -> np.ndarray:
		...
		while i<len(population): 
			...
			if np.random.rand(1)[0] < self.p_crossover:
				...
				# update two memembers of the population (if enough left) and if already not in new pop
				new_pop[i][0] = child1
				i+=1
				if i<len(population): 
					new_pop[i][0] = child2
	...
	def _crossover_then_mutate(self, parent1: str, parent2: str) -> Tuple[str,str]:
		...
		for i in range(0,self.space.parameters[0].length):
			...
			if np.random.rand(1)[0] < self.p_mutation:
				parent1[i] = np.random.choice(self.possible_codon_swaps[i])
			# see if need to mutate child2 at poistion i
			if np.random.rand(1)[0] < self.p_mutation:
				parent2[i] = np.random.choice(self.possible_codon_swaps[i])
	...
	def _reproduce_then_mutate(self, parent: str,) -> str:
		...
		for i in range(0,self.space.parameters[0].length):
			# see if need to mutate child1 at poistion i
			if np.random.rand(1)[0] < self.p_mutation:
				parent[i] = np.random.choice(self.possible_codon_swaps[i])
		return " ".join(parent)
