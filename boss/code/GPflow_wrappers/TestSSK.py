...
class TestSSK(Kernel):
   ...
   with hyperparameters:
    ...
    def __init__(self,W=None,kappa=None,rank=1,active_dims=[0],gap_decay=0.1, match_decay=0.9,max_subsequence_length=3,
                 alphabet = [],swapped_alphabet=[],maxlen=0):
        ...
        self.swapped_alphabet = tf.constant(swapped_alphabet)
    ...
    def K_diag(self, X):
        ...
        K = self.K(X1,X2)
        return tf.linalg.diag_part(K)[:-1]
    ...
    def K(self, X1, X2=None):
        ...
        X1 = tf.transpose(tf.gather(tf.transpose(X1),[3,2,1,0]))
