...
class Batch_FSSK(Kernel):
   ...
   with hyperparameters:
    ...
    def kernel_calc(self,X_batch,X2_batch):
        ...
        _ = tf.square(self.order_coefs_param)
        _ = tf.square(self.kappa_param)
        _ = tf.square(self.W_param)


        if self.symmetric:
            k, dk_dgap, dk_dmatch,dk_dorder_coefs, dk_dkappa, dk_dW = self.kernel_calc_with_grads(X_batch,X2_batch)
        else:
        ...
        def grad(dy, variables=None):
            ...
            if self.symmetric:
                ...
                dy_tiled = tf.tile(tf.expand_dims(dy,1),(1,self.max_subsequence_length))
                grads['order_coefs:0'] = tf.reduce_sum(tf.multiply(dy_tiled,dk_dorder_coefs*tf.math.exp(positive().forward_log_det_jacobian(self.order_coefs_unconstrained,0))),0)
    ...
    def kernel_calc_with_grads(self,X_batch,X2_batch):
        ...
        dKp_dcomb = tf.TensorArray(tf.float64,size=self.max_subsequence_length ,clear_after_read=False)
        ...
        dKp_dcomb = dKp_dcomb.write(0,tf.zeros(shape=tf.stack([self.alphabet_size*(1+self.rank),tf.shape(S)[0], self.maxlen,self.maxlen]), dtype=tf.float64))
        ...
        for i in tf.range(self.max_subsequence_length-1):
            ...
            dKp_dcomb_temp = tf.multiply(dS_dcomb, Kp.read(i)) 
            dKp_dcomb_temp = dKp_dcomb_temp + tf.multiply(S, dKp_dcomb.read(i))
            dKp_dcomb_temp = dKp_dcomb_temp * match_sq
            dKp_dcomb_temp = tf.matmul(dKp_dcomb_temp,self.D)
            dKp_dcomb_temp = tf.matmul(self.D,dKp_dcomb_temp,transpose_a=True)
            dKp_dcomb = dKp_dcomb.write(i+1,dKp_dcomb_temp)
        ...
        dKp_dmatch.close()
        dKp_dcomb_stacked = dKp_dcomb.stack()
        dKp_dcomb.close()     
        ...
        # get order coefs grads
        dk_dorder_coefs = tf.transpose(Ki)



        # get kappa and W grads
        temp = tf.multiply(tf.expand_dims(tf.expand_dims(S,0),0), dKp_dcomb_stacked)
        temp = temp + tf.multiply(tf.expand_dims(dS_dcomb,0), tf.expand_dims(Kp_stacked,1))
        temp = tf.reduce_sum(temp, -1)
        temp = tf.reduce_sum(temp, -1)
        temp = temp * match_sq
        dk_dcomb  = tf.linalg.matvec(tf.transpose(temp),self.order_coefs)

        dk_dkappa = dk_dcomb[:,:self.alphabet_size]
        dk_dW = dk_dcomb[:,self.alphabet_size:]


        return k, dk_dgap, dk_dmatch, dk_dorder_coefs, dk_dkappa, dk_dW
