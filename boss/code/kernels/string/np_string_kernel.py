...
class NPStringKernel(object):
	...
	def K_no_grads(self, X, X2=None):
		...
		#if needed calculate the values needed for normalization
		if self.normalize:
			X_diag_Ks = self._diag_calculations_no_grads(X)
			if not symmetric:
				X2_diag_Ks = self._diag_calculations_no_grads(X2)
		...
		for i, x1 in enumerate(X):
			for j, x2 in enumerate(X2):
				...
				if symmetric and (j < i):
					...
					k_result = _k_no_grads(x1[0],x2[0],self.embs, self.maxlen, self._gap_decay, self._match_decay, np.array(self._order_coefs),self.D,self.dD_dgap) 

					# Normalize kernel and gradients if required
					if self.normalize:
						if symmetric:
							k_result_norm = _normalize_no_grads(k_result,X_diag_Ks[i], X_diag_Ks[j])
						else:
							k_result_norm = _normalize_no_grads(k_result,X_diag_Ks[i], X2_diag_Ks[j])
						k_results[i, j] = k_result_norm
					else:
						k_results[i, j] = k_result
		return k_results
	...
	def _diag_calculations_no_grads(self, X):
		"""
		Calculate the K(x,x) values first because
		they are used in normalization.
		This is pre-normalization (otherwise diag is just ones)
		...
		# All set up. Proceed with kernel matrix calculations
		for i, x1 in enumerate(X):
			result = _k_no_grads(x1[0],x1[0],self.embs, self.maxlen, self._gap_decay, self._match_decay, np.array(self._order_coefs),self.D,self.dD_dgap)  
			k_result[i] = result
		return k_result
...
def _normalize_no_grads(K_result,diag_Ks_i,diag_Ks_j):
	"""
	Normalize the kernel and kernel derivatives.
	Following the derivation of Beck (2015)
	"""
...
def _k_no_grads(  s1,  s2,  embs,  maxlen,  _gap_decay,  _match_decay, _order_coefs,D,dD_dgap):
	"""
	TF code for vecotrized kernel calc.
	Following notation from Beck (2017), i.e have tensors S,D,Kpp,Kp
