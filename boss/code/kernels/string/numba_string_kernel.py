...
class numbaStringKernel(object):
    """
...
global_types = (int32[:],int32[:],float64[:,:],int32,float64,float64,float64[:],float64[:,:],float64[:,:])


@jit(global_types,nopython=True) 
def _k(  s1,  s2,  embs,  maxlen,  _gap_decay,  _match_decay, _order_coefs,D,dD_dgap):
    ...
    for i in range(len(_order_coefs)-1):
        aux1 = S * Kp[i]
        aux2 = np.dot(aux1,D[0:maxlen, 0:maxlen])
        Kpp = match_sq * aux2
